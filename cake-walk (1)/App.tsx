...
import { initializeOneSignal, requestNotificationPermission, isUserSubscribed, setUserPreferences } from './onesignal-config';
import { initializeReviewNotifications } from './supabase-notifications';
import { scheduleDailyBirthdayCheck, scheduleAtSpecificTime } from './birthday-notifications';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const initPushNotifications = async () => {
      try {
        ...
        if (typeof window !== 'undefined' && (window as any).OneSignal) {
          ...
          // Initialize Supabase realtime for review notifications
          initializeReviewNotifications();
          
          // 🎂 Initialize Birthday Notifications
          // Check for birthdays daily at 9:00 AM
          scheduleAtSpecificTime(9, 0);
          
          console.log('✅ Push notifications initialized');
          console.log('🎂 Birthday notifications scheduled');
        }
      } catch (error) {
    ...
    // Delay to ensure OneSignal SDK is loaded
  ...
  // Save profile
  const handleSaveProfile = async (name: string, newGender: 'male' | 'female') => {
      setUsername(name);
      setGender(newGender);
      ...
      // Update OneSignal user preferences
      if ((window as any).OneSignal) {
        await setUserPreferences(name, 'user');
      }
  };
